    ...
    (entries, observer) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          const data = bg.dataset;
          const src = (isMobile && data.srcMobile) || data.src;
...
function genVideoAttr(data){
  const controls = data.controls || 0;
  const mute = data.mute || 1;
  const loop = data.loop || 1;
  const autoplay = data.autoplay || 1;
  return `controls=${controls}&autoplay=${autoplay}&mute=${mute}&loop=${loop}`;
}
...
function initializeLazyLoad() {
  ...
  const loadYouTubeVideo = (element) => {
    ...
    const attrStr = genVideoAttr(data);
    iframe.src = `https://www.youtube.com/embed/${videoId}?&playsinline=1&enablejsapi=1&playlist=${videoId}&${attrStr}`;
  ...
  const loadVimeoVideo = (element) => {
    ...
    iframe.src = `https://player.vimeo.com/video/${videoId}?background=1&${attrStr}`;
    iframe.allowFullscreen = true;
...
class MultiContentWW extends SlideSection {
  constructor() {
    super();
    this.enable = this.dataset.enable;
    this.swiperSlideInnerHtml = this.innerHTML;
...
customElements.define('multi-content-ww', MultiContentWW);
...
function getScrollBar() {
  const w = getScrollBarWidth.init()
  document.body.style.setProperty('--scroll-w', w + 'px')
}
getScrollBar()



//WW adding for custom function



class PopupAddons extends PopupBase {
  constructor() {
    super();
    this.popupPopupDiv = this.querySelector(".popup-addons__content");
    this.init();
  }
  init() {
    if (this.popupPopupDiv) {
      this.addEventListener("click", this.onClick.bind(this));
    }
  }
  onClick() {
    const html = this.popupPopupDiv;
    if (!html) return;
    html.classList.remove("hidden");
    this.initPopup(
      html,
...
customElements.define("popup-addons", PopupAddons);

class WWTabItems extends HTMLElement {
  constructor() {
    super();
    this.tabs = this.querySelectorAll('.ww-tab__tab-item');
    this.select = this.querySelector('.ww-tab__select-tab');
    this.content = this.querySelectorAll('.ww-tab__tab-content');
    this.init();
  }
  init() {
    const self = this;
...
customElements.define('ww-tab-items', WWTabItems);

// js for tab item
class WWTabMulti extends HTMLElement {
  constructor() {
    super();
    this.select = this.querySelector('.ww-select-collection-tab');
    this.content = this.querySelectorAll('.ww-collection-tab__tab-content');
    this.select_li = this.querySelectorAll('.ww-collection_title_input');
    this.init();
  }
  init() {
    const self = this;
    this.select.addEventListener('click', function (e) {
      if (e.target.classList.contains('ww-select__selected_title')) {
        let select__dropdown = this.querySelector('.ww-select-custom__content');
        this.classList.toggle('active');
        select__dropdown.classList.toggle('invisible');
      }
    });
    document.addEventListener('click', function (event) {
      if (!event.target.closest('.ww-select-collection-tab')) {
        self.select.classList.remove('active');
        document
          .querySelectorAll('.ww-select-collection-tab .ww-select-custom__content')
          ?.forEach((item) => {
            item.classList.add('invisible');
          });
      }
  ...
  onClickBySelect(selectedBlockId) {
    let select__selected_title = this.querySelector('.ww-select__selected_title');
    select__selected_title.innerHTML =
      selectedBlockId.innerHTML +
      `<svg class="icon-down transition active-rotated" width="10" height="6">
    <use href="#icon-arrow-down"></use>
...
customElements.define('ww-multi-tab-select', WWTabMulti);